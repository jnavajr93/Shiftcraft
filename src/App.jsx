...
export default function App() {
  ...
  // Solver output — recomputes whenever config or active week changes.
  const solverResult = useMemo(() => {
    try { return solve(cfg, week); } catch (e) { console.error(e); return {}; }
  }, [cfg, week]);

  // displayResult is a mutable copy of solverResult. Drag-to-move edits it
  // directly without touching the config. It resets whenever the solver reruns.
  const [displayResult, setDisplayResult] = useState(solverResult);
  useEffect(() => { setDisplayResult(solverResult); }, [solverResult]);

  // Move a person chip from one shift to another (day + index based).
  ...
  // Update shift start/end times in the config (triggers solver rerun + display reset).
            ...
            <div className="section-head">
              ...
              {hasWeekTags && (
                <div className="week-toggle">
                  <button className={'week-btn' + (week === 'A' ? ' active' : '')} onClick={() => setWeek('A')}>Week A</button>
                  <button className={'week-btn' + (week === 'B' ? ' active' : '')} onClick={() => setWeek('B')}>Week B</button>
                </div>
              )}
              <button
              ...
              <span className="hint">Drag chips to reassign · click times to edit · flex shows staffing gaps</span>
            </div>
            <Board
