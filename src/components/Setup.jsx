      ...
      <div className="grid-cards">
        {cfg.shifts.map((s) => (
          <div className="card" key={s.id}>
            ...
            <div className="field">
              <label>Week <span style={{ color: 'var(--ink-faint)', fontWeight: 400 }}>(both = runs every week)</span></label>
              <div className="pill-group">
                {[['Both', null], ['A only', 'A'], ['B only', 'B']].map(([label, val]) => (
                  <button key={label}
                    className={'pill' + (s.week === val ? ' on' : '')}
                    onClick={() => edit(s.id, { week: val })}>
                    {label}
                  </button>
                ))}
              </div>
