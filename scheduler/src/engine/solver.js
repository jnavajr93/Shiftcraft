...
export function solve(cfg) {
  ...
  for (const day of activeDays(cfg)) {
    const used = new Set(); // personIds already placed today
    const dayShifts = cfg.shifts.filter((s) => (s.days || []).includes(day));
    const cards = [];
    const issues = [];
...
function activeDays(cfg) {
  const set = new Set();
  cfg.shifts.forEach((s) => (s.days || []).forEach((d) => set.add(d)));
