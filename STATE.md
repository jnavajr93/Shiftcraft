...
### Dr. B / Yadi staffing rule
- Yadi: `roles: ['Opener']`, `lockedTo: [{ provider: 'Dr. B', slot: 'scribe' }]` (seed.js:513–514)
- The `Scribe` role was **removed** from Yadi's roles array so the free-candidacy solver can't pick her up at other clinics
- Solver enforces MUST_PAIR via the adapter, placing Yadi as Dr. B's scribe before filling other slots
- `availabilityWindows` limits Yadi to Mon/Wed/Thu/Fri with specific end-no-later times
...
## Absence System (`src/utils/absenceUtils.js`)

### Blocking vs non-blocking types
Non-blocking (stored in `NON_BLOCKING = new Set(['DoctorOff', 'Research'])`):  
- `DoctorOff` — informational; staff can still be assigned  
- `Research` — marks research rotation; does not block other assignments

Everything else blocks: Approved Time Off, Sick, Unpaid, etc.

### Partial-day overlap rule
Full-day absence (`partial_start == null && partial_end == null`): always blocks the day.  
Partial absence: strict interval overlap — `slotStart < absenceEnd && slotEnd > absenceStart`. Touching boundaries do NOT block.

### Blocking paths (all three enforce absence check)
1. **Drag-and-drop** (`App.jsx handleDragEnd`) — checks absence before applying
2. **Slot popover** (`SlotPopover.jsx ineligibleReason`) — greys out absent staff with reason label
3. **AppContext `assignSlot` / `assignTask`** — hard guard, rejects assignment even if somehow called directly

### Display
`UnassignedStaff.jsx` splits into: Available (unassigned, can work), Unavailable (absent — greyed, non-interactive), and silently hides permanent day-off staff.
